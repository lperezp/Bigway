...
geoXML3.parser = function(options) {
  ...
  var parse = function(urls, docSet) {
    ...
    if (!parserName) {
      parserName =
        "geoXML3.instances[" + (geoXML3.instances.push(this) - 1) + "]";
    }
  ...
  function fetchDoc(url, doc, resFunc) {
    ...
    if (
      ...
      typeof JSIO === "object" &&
      typeof JSIO.guessFileType === "function"
    ) {
  ...
  var showDocument = function(doc) {
    ...
    if (!!doc.ggroundoverlays) {
      for (i = 0; i < doc.ggroundoverlays.length; i++) {
        doc.ggroundoverlays[i].setOpacity(
          doc.ggroundoverlays[i].percentOpacity_
        );
      }
    }
    if (!!doc.gpolylines) {
  ...
  function processStyle(thisNode, baseUrl, styleID, baseDir) {
    var style =
      baseUrl === "{inline}"
        ? clone(defaultStyle)
        : (styles[baseUrl][styleID] =
            styles[baseUrl][styleID] || clone(defaultStyle));

    var styleNodes = getElementsByTagName(thisNode, "BalloonStyle");
    if (!!styleNodes && styleNodes.length > 0) {
      style.balloon.bgColor = nodeValue(
        getElementsByTagName(styleNodes[0], "bgColor")[0],
        style.balloon.bgColor
      );
      style.balloon.textColor = nodeValue(
        getElementsByTagName(styleNodes[0], "textColor")[0],
        style.balloon.textColor
      );
      style.balloon.text = nodeValue(
        getElementsByTagName(styleNodes[0], "text")[0],
        style.balloon.text
      );
      style.balloon.displayMode = nodeValue(
        getElementsByTagName(styleNodes[0], "displayMode")[0],
    ...
    if (!!styleNodes && styleNodes.length > 0) {
      ...
      icon.scale = parseFloat(
        nodeValue(getElementsByTagName(styleNodes[0], "scale")[0], icon.scale)
      );
      ...
      if (!!styleNodes && styleNodes.length > 0) {
        ...
        icon.dim = {
          x: parseInt(
            nodeValue(
              getElementsByTagNameNS(styleNodes[0], gxNS, "x")[0],
              icon.dim.x
            )
          ),
          y: parseInt(
            nodeValue(
              getElementsByTagNameNS(styleNodes[0], gxNS, "y")[0],
              icon.dim.y
            )
          ),
          w: parseInt(
            nodeValue(
              getElementsByTagNameNS(styleNodes[0], gxNS, "w")[0],
              icon.dim.w
            )
          ),
          h: parseInt(
            nodeValue(
              getElementsByTagNameNS(styleNodes[0], gxNS, "h")[0],
              icon.dim.h
            )
          )
        };
        ...
        if (
          true /* (icon.dim.w < 0 || icon.dim.h < 0) && (icon.xunits != 'pixels' || icon.yunits == 'fraction') || icon.scale != 1.0 */
        ) {
    ...
    if (!!styleNodes && styleNodes.length > 0) {
      style.line.color = nodeValue(
        getElementsByTagName(styleNodes[0], "color")[0],
        style.line.color
      );
      style.line.colorMode = nodeValue(
        getElementsByTagName(styleNodes[0], "colorMode")[0],
        style.line.colorMode
      );
      style.line.width = nodeValue(
        getElementsByTagName(styleNodes[0], "width")[0],
        style.line.width
      );
    ...
    if (!!styleNodes && styleNodes.length > 0) {
      ...
      style.poly.outline = getBooleanValue(
        getElementsByTagName(styleNodes[0], "outline")[0],
        style.poly.outline
      );
      style.poly.fill = getBooleanValue(
        getElementsByTagName(styleNodes[0], "fill")[0],
        style.poly.fill
      );
    }
    return style;
  ...
  function processStyleMap(thisNode, baseUrl, styleID, baseDir) {
    ...
    for (var pr = 0; pr < pairs.length; pr++) {
      ...
      var pairStyleBaseUrl = pairStyleUrl[0]
        ? cleanURL(baseDir, pairStyleUrl[0])
        : baseUrl;
  ...
  var render = function(responseXML, doc) {
    ...
    if (!responseXML || responseXML == "failed parse") {
      ...
      for (i = 0; i < nodeCount; i++) {
        ...
        if (!!styleID)
          processStyle(thisNode, doc.baseUrl, styleID, doc.baseDir);
      }
      ...
      for (i = 0; i < nodes.length; i++) {
        ...
        var styleID = thisNode.getAttribute("id");
        if (!!styleID)
          processStyleMap(thisNode, doc.baseUrl, styleID, doc.baseDir);
      }
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        placemark = {
          ...
          styleBaseUrl: styleUrl[0]
            ? cleanURL(doc.baseDir, styleUrl[0])
            : doc.baseUrl,
          styleID: styleUrl[1],
          visibility: getBooleanValue(
            getElementsByTagName(node, "visibility")[0],
            true
          ),
          balloonVisibility: getBooleanValue(
            getElementsByTagNameNS(node, gxNS, "balloonVisibility")[0],
            !parserOptions.suppressInfoWindows
          ),
          id: node.getAttribute("id")
        };
        placemark.style =
          (styles[placemark.styleBaseUrl] &&
            styles[placemark.styleBaseUrl][placemark.styleID]) ||
          clone(defaultStyle);
        ...
        if (!!extDataNodes && extDataNodes.length > 0) {
          ...
          for (var d = 0; d < dataNodes.length; d++) {
            ...
            if (!name) continue;
            var dName = nodeValue(
              getElementsByTagName(dn, "displayName")[0],
              name
            );
            ...
            if (
              ...
              switch (Geometry) {
                case "Point":
                  ...
                  placemark.latlng = new google.maps.LatLng(
        ...
        if (TrackNodes.length > 0) {
          for (var i = 0; i < TrackNodes.length; i++) {
            var coordNodes = getElementsByTagNameNS(
              TrackNodes[i],
              gxNS,
              "coord"
            );
      ...
      for (i = 0; i < groundNodes.length; i++) {
        ...
        groundOverlay = {
          ...
          latLonBox: {
            north: parseFloat(
              nodeValue(getElementsByTagName(node, "north")[0])
            ),
            east: parseFloat(nodeValue(getElementsByTagName(node, "east")[0])),
            south: parseFloat(
              nodeValue(getElementsByTagName(node, "south")[0])
            ),
            west: parseFloat(nodeValue(getElementsByTagName(node, "west")[0]))
          },
          rotation:
            -1 *
            parseFloat(nodeValue(getElementsByTagName(node, "rotation")[0]))
        };
        if (!!google.maps) {
          doc.bounds = doc.bounds || new google.maps.LatLngBounds();
          doc.bounds.union(
        ...
        if (!!doc) {
          ...
          if (doc.reload) {
            ...
            for (i = overlays.length; i--; ) {
              if (
                overlays[i].bounds().equals(overlayBounds) &&
      ...
      for (i = 0; i < linkNodes.length; i++) {
        ...
        if (networkLink.link.refreshMode === "onInterval") {
          networkLink.link.refreshInterval = parseFloat(
            nodeValue(getElementsByTagName(node, "refreshInterval")[0])
          );
          if (isNaN(networkLink.link.refreshInterval)) {
            networkLink.link.refreshInterval = 0;
          }
        } else if (networkLink.link.refreshMode === "onChange") {
          networkLink.link.viewRefreshMode = nodeValue(
            getElementsByTagName(node, "viewRefreshMode")[0]
          );
          if (!networkLink.link.viewRefreshMode) {
            networkLink.link.viewRefreshMode = "never";
          }
          if (networkLink.link.viewRefreshMode === "onStop") {
            networkLink.link.viewRefreshTime = nodeValue(
              getElementsByTagName(node, "refreshMode")[0]
            );
            networkLink.link.viewFormat = nodeValue(
              getElementsByTagName(node, "refreshMode")[0]
            );
        ...
        // Apply the link
        if (
          networkLink.link.refreshMode === "onInterval" &&
          networkLink.link.refreshInterval > 0
        ) {
          // Reload at regular intervals
          setInterval(
            parserName + '.parse("' + networkLink.link.href + '")',
    ...
    if (!!doc.bounds) {
      doc.internals.bounds =
        doc.internals.bounds || new google.maps.LatLngBounds();
    ...
    if (!doc.internals.parseOnly) {
      ...
      if (doc.baseUrl) {
        // handle case from parseKmlString (no doc.baseUrl)
  ...
  var processStyleID = function(style) {
    ...
    var rnd = Math.round;
    var y = icon.dim.y;
    if (typeof icon.dim.th !== "undefined" && icon.dim.th != icon.dim.h) {
      // palette - reverse kml y for maps
    ...
    var iconSize = icon.dim.whGuess
      ? null
      : new google.maps.Size(rnd(scaled.w), rnd(scaled.h));
    var iconScale =
      icon.scale == 1.0
        ? null
        : icon.dim.whGuess
  ...
  var createInfoWindow = function(placemark, doc, gObj) {
    ...
    if (
      ...
      var address = encodeURIComponent(
        vars.val.address || placemark.latlng.toUrlValue()
      ).replace(/\%20/g, "+");
      ...
      vars.val.geDirections =
        ...
        "&daddr=" +
        address +
        '" target=_blank>To Here</a> - <a href="' +
        url +
        "&saddr=" +
        address +
    ...
    var iwText = bStyle.text.replace(/\$\[(\w+(\/displayName)?)\]/g, function(
      txt,
      n,
      dn
    ) {
    ...
    var styleProp = styleArr.length
      ? ' style="' + styleArr.join(" ") + '"'
      : "";

    var infoWindowOptions = geoXML3.combineOptions(
      parserOptions.infoWindowOptions,
      {
        content:
    ...
    gObj.infoWindow =
      parserOptions.infoWindow || new google.maps.InfoWindow(infoWindowOptions);
    gObj.infoWindowOptions = infoWindowOptions;

    // Info Window-opening event handler
    //evento que salta cuando haces clic en el polygon
    google.maps.event.addListener(gObj, "click", function(e) {
      ...
      iW.setContent(
        "<div id='geoxml3_infowindow'>" + iW.getContent() + "</div>"
      );
      // ESTA LINEA SE AGREGA PARA QUE MUESTRE EN CONSOLA LA ZONA QUE ESTA UBICADA EL MARCADOR
      console.log("Esta zona es: ", gObj.title);
    ...
    function hola() {
      gObj.addListener("click", function(e) {
        var iW = this.infoWindow;
        iW.close();
        iW.setOptions(this.infoWindowOptions);
        ...
        // ESTA LINEA SE AGREGA PARA QUE MUESTRE EN CONSOLA LA ZONA QUE ESTA UBICADA EL MARCADOR
        console.log("Este barrio es: ", gObj.title);
        iW.open(this.map, this.bounds ? null : this);
      });
    }
    // fin del la funcion
...
geoXML3.fetchXML = function(url, callback) {
  ...
  if (!!geoXML3.fetchers.length) xhrFetcher = geoXML3.fetchers.pop();
    ...
    xhrFetcher.fetcher = new window.XMLHttpRequest();
  // Most browsers
  ...
  xhrFetcher.fetcher.onreadystatechange = function() {
    if (xhrFetcher.fetcher.readyState === 4) {
      ...
      if (xhrFetcher.fetcher.status >= 400) {
        geoXML3.log(
          "HTTP error " + xhrFetcher.fetcher.status + " retrieving " + url
        );
        callback();
      }
      // Returned successfully
        ...
        if (xhrFetcher.fetcher.responseXML) {
          ...
          // handle valid xml sent with wrong MIME type
  ...
  var zipFile = new ZipFile(url, function(zip) {
    ...
    for (var i = 0; i < zip.status.length; i++) {
      ...
      if (msg.indexOf("ERROR") == 0) {
        ...
        // non-fatal, but still might be useful
    ...
    var kmlProcessing = {
      // this is an object just so it gets passed properly
      timer: null,
      extractLeft: 0,
      timerCalls: 0
    };
    var extractCb = function(entry, entryContent) {
      ...
      if (
        ...
        geoXML3.log(
          "KMZ error extracting " + mdUrl + ": " + entryContent.description
        );
        callback();
        return;
      }
      ...
      parser.kmzMetaData[mdUrl].dataUrl =
        "data:" + mime + ";base64," + base64Encode(entryContent);
      ...
      if (
        ...
        if (
          ...
          (IEversion < 9.0 && parser.kmzMetaData[mdUrl].dataUrl.length > 32767)
        ) {
          parser.kmzMetaData[mdUrl].dataUrl =
    ...
    var kmlExtractCb = function(entry, entryContent) {
      if (
        ...
        geoXML3.log(
          "KMZ error extracting " + entry.name + ": " + entryContent.description
        );
        callback();
        return;
      }
      ...
      if (kmlProcessing.extractLeft <= 1) {
        ...
        if (kmlProcessing.timerCalls < 100) {
          ...
          geoXML3.log(
            "KMZ warning extracting " +
              url +
              ": entire ZIP has not been extracted after 10 seconds; running through KML, anyway..."
          );
          kmlProcessing.extractLeft--;
          callback(geoXML3.xmlParse(entryContent));
        }
...
geoXML3.getElementsByTagNameNS = function(node, namespace, tagname) {
  if (node && typeof node.getElementsByTagNameNS != "undefined")
    ...
    if (attr.prefix === "xmlns" && attr.nodeValue === namespace)
      ...
      if (typeof node.selectNodes != "undefined") {
        ...
        if (!root.ownerDocument.getProperty("SelectionNamespaces"))
          root.ownerDocument.setProperty(
            "SelectionNamespaces",
...
geoXML3.getElementsByTagName = function(node, tagname) {
  if (node && typeof node.getElementsByTagNameNS != "undefined")
    return node.getElementsByTagName(tagname); // if it has both functions, it should be accurate
